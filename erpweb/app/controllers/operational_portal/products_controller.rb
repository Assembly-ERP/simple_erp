...
module OperationalPortal
  class ProductsController < ApplicationController
    ...
    def update_product_parts(product, part_quantities)
      parts_products_table = if ActiveRecord::Base.connection.table_exists?(:parts_products)
                               :parts_products
                             else
                               'parts_products'
                             end
      ...
      part_quantities.each do |part_id, quantity|
        ...
        ActiveRecord::Base.connection.execute(
          "UPDATE #{parts_products_table} SET quantity = #{ActiveRecord::Base.connection.quote(quantity)}" \
          "WHERE product_id = #{product.id} AND part_id = #{part_id}"
        )
      end
    end
